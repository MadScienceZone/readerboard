...
The current version of the hardware expands on the version 2.1.0 design by replacing the single-color \acronym{LED}s with
...
Version 3.2.1 expands the number of logical rows due to the \acronym{RGB} implementation, which requires
a matching firmware implementation and changes to the protocol to support the new color data.

At this point in development it is unclear whether we'll continue making the different versions
...
Every message has a field called \z{"m"} which contains the message type as a string. Each message contains
additional fields as needed for its message type. Any expected fields which are missing are assumed to contain
the ``zero'' value for their type (\z{false}, \z0, \z{""}, etc.) and unexpected fields which are presented are
silently ignored.
\subsection{Message Target Addresses}
Every message also has a field called \z{"a"} which is a list of target readerboards to which the message
applies. This is always a list even if it has a single target.
...
If multiple devices are targeted, say with a message containing the \acronym{JSON} field \z{"a":[2,5,37]} with address 15 (e.g., \z{F}$_{16}$) configured as the global address \Var{ad$_G$},
...
\begin{table}
	\begin{center}
		\begin{tabular}{lcll}\toprule
			...
			\z{query}&\z{Q} & Query matrix display status & [2] [4] [6]\\
			...
			\multicolumn{4}{l}{\footnotesize [5] Only if field \z{"status"} is \z{true}}\\
			\multicolumn{4}{l}{\footnotesize [6] Only if field \z{"status"} is omitted or \z{false}}\\
		\end{tabular}
...
Sets the column cursor position to the value indicated by \Var*{pos}. See Table~\ref{tbl:int063}. (The \acronym{JSON} message uses integer values for \Var*{col} without the encoding used for \Var*{pos}.)
