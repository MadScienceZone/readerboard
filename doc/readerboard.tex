...
\begin{center}\bfseries
	For readerboard hardware (\acronym{PCB}) version 3.3.0, schematic version 3.3.0, and firmware version 2.1.3.
\end{center}
...
The busylight can drive up to seven \acronym{LED}s. If directly connected to a host computer
...
\begin{enumerate}
	...
	\item Build each \acronym{LED} tier of 8 \acronym{LED}s:
		\begin{itemize}
			\item Solder eight 3\,mm \acronym{LED}s around the edge of the board with their leads straddling
			...
			\item Solder the two resistor arrays, making sure to select the correct values for the specific
				\acronym{LED}s used on this tier board.
		\end{itemize}
...
%Also, unlike \z{S}, the \acronym{LED}s specified are \emph{added to} the ones which may already
%be lit. To clear the existing \acronym{LED}s first, issue the \z{X} command ahead of \z{L}.
...
Edit a few lines near the top of \z{readerboard.h}:
\begin{SourceCode}
...
\end{SourceCode}
These are set for the standard color assignments for the discrete status \acronym{LED}s, allowing for
the use of these single-letter color codes to refer to the status \acronym{LED}s. If you build a
...
#define BANNER_SERIAL_NUMBER "S/N RBXXXX"
#define SERIAL_VERSION_STAMP "V3.2.3$R0.0.0$SRBXXXX$"
//                             \___/  \___/  \____/
//                               |      |       |
//                  Hardware version    |       |
//                         Firmware version     |
...
\noindent\includegraphics[width=.48\textwidth]{images/busylight_tree-assembly-0.png}
%\noindent\includegraphics[width=.48\textwidth]{images/busylight_tree-assembly-1.png}
\indexintoc
