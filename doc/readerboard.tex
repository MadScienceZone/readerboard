% NEED	ORDER
% 5	-	J0 stacking pins: 1x10, 5x8
% 5	-	J1a 179-TBLH10-381-08BK	or 6-pin
% 10	-	J2,J3 2p header
% 5	-	J4 3p header
% 5	-	J5 2p euro terminal block
% 6	-	J6 10p IDC header | 10p w2b header
% 6	-	10p IDC socket    | 10p w2b plug w/pins
% 10	-	J7,J8 16p IDC header | half 16p w2b header
% 10	-	16p IDC socket       | half 16p w2b plug w/pins
% 5	-	C0 10uF elec	(2560)
% 5	-	C1 0.1uF 
% 5 	-	D0 2A
% 25	-	R0-R4 10K
...
% 10	-	U2,3 ULN2003
%
% Nameplate R built-in jumper
% Nameplate G built-in jumper
% Nameplate B built-in jumper
% Button A 12V white 3v 450
% Button B 12V white 3v 450
% Button C 12V white 3v 450
% Button D 12V white 3v 450
% Button X R 12V 2v 500
% Button X G 12V 3v 450
% Button X B 12V 3v 450
% Button L R 12V 2v 500
% Button L Y 12V 2v 500
% Button L G 12V 3v 450
% Heartbeat B 12V 3v 450
%
% 16	-	white square button w/LED ABCD
% 4	-	lart switch
% 4	0	sets of r/g/y led side lights
% 4 	-	large round white button X
% 4 	-	RGB LED for X buttons
% 8	-	adafruit backpack segment displays
% 4	-	RGB nameplate lights
% 8	-	power connections? PSU?
% 
%
% 30	-	1N4148 diode
% 10	0	female centronics PCB 90deg sockets
% 5	-	8p IDC header | 8p w2b header
% 5	-	8p IDC socket | 8p w2b plug w/pins
% 5	-	10p euro terminal block
% 10-20	-	36p stacking headers | half sockets only w/o pins
...
% TODO: ^Jp in strings
% TODO: ^Up in strings
% TODO: H takes 0-63 value
% TODO: HK takes $-terminated list
% TODO: I takes height parameter
% TODO: Q returns matrix dimensions
% TODO: {up} and {down}
\input common
\frontmatter
...
\newcommand\ltp{\acronym{LTP}}
\newcommand\msb{\acronym{MSB}}
...
\thispagestyle{empty}
	\begin{center}
		{\Huge Readerboard and Busylight \\ User's Guide}

		\bigskip

		{\large Includes details on assembly, installation, and programming these messaging units.}

	\end{center}

\vfill
\begin{center}
\includegraphics[width=\textwidth]{images/readerboard_photo.png}

\includegraphics[width=.5\textwidth]{images/busylight_photo.png}
\end{center}
\vfill
%\end{flushright}
\newpage
...
\begin{center}\bfseries
	For Readerboard hardware (\acronym{PCB}) version 3.4.2, 
	Busylight hardware version 2.2.1 (light tree version 1.1), and firmware version 2.3.8.

	(Experimental support for Adafruit 16$\times$32, 32$\times$32, 16$\times$32$\times$2, and 64$\times$64 \acronym{RGB} matrix displays is a work in progress
	with current \acronym{PCB} revision 0.0.0.)
\end{center}
...
\subsection{Version 3.4.1 (current model)}
Version 3.4 added support for playing sounds on a speaker and added a diode (D520) to prevent
the Arduino from trying to power the \led s by itself if it is powered from the \acronym{USB}
port but the external 9\,V supply is not active.

\subsection{Version 3.3.1}
Version 3.3 of the hardware corrects some minor issues with the 3.2.2 revision by adding a cutout to avoid colliding with the
...
\begin{table}
	\begin{tabular}{lrll}\toprule
		...
		D0--511&512&[1]&\led, 5mm, \acronym{RGB}, common cathode\\
		D512&1&&\led, 5mm, green\\
		D513--14&2&&\led, 5mm, amber (yellow)\\
		D515--16&2&&\led, 5mm, red\\
		D517--18&2&&\led, 5mm, blue\\
		D519&1&&\led, 5mm, white\\
		D520&1&&power diode, 4\,A\\
		J0&1&&Arduino stacking pin set\\
		J1a,b&2&\mouser{179-TBLH10-381-08BK}&8-position terminal block\\
		J2,3&2&\mouser{649-69190-102HLF}&2-position jumper header\\
		J4&1&&3-position jumper header\\
		J5,6&2&&2-position Euro-style terminal block\\
		P0--2&3&&2-position jumper shunt\\
		...
		U11&1&\mouser{595-THVD1439VDR}&\mc{THVD1439} half-duplex RS-485 driver/receiver\\
		U16&1&\mouser{579-24LC256T-E/SN}&\mc{AT24C256} \acronym{EEPROM}\\
		%X0&1&&\mc{AT24C256} \acronym{EEPROM} module\\
		&24&&\mc{TO}-220 heat sinks for Q0--23 (optional)\\
		&1&&9V 2A DC power supply\\
		\bottomrule
		\multicolumn{4}{p{\textwidth}}{\tiny[1] I found these at Amazon.com: \href{https://www.amazon.com/EDGELEC-Tri-Color-Multicolor-Diffused-Resistors/dp/B077XGF3YR}{EDGELEC 5mm RGB Tri-Color 4Pin LED Diodes Common Cathode}.}
	\end{tabular}
...
\begin{enumerate}
	\item Solder all the surface mount chips first: U0--U15 (and optionally, U16).
		...
		\end{itemize}
	\item Solder the discrete resistors: R0--R7, R24--R51, and R54 (and optionally, R52 and R53). See below first to ensure the correct values are used
		...
		\begin{itemize}
			...
			\item If using sound output, solder in R54: 10K
		\end{itemize}
	...
	\item Solder the Arduino bus stacking pins (six connectors---1$\times$10-pin, 5$\times$8-pin---all labeled collectively as ``J0'').
...
\begin{table}
    \begin{center}
	\begin{tabular}{lrll}\toprule
		{\bfseries Part}&{\bfseries Qty}&&{\bfseries Description}\\\midrule
		...
		C0&1&&\SI{0.1}{\micro\farad} capacitor\\
		C1&1&&\SI{100}{\micro\farad} capacitor\\
		D0&1&&power diode, 1\,A\\
		J0&1&&8-pin DIN connector, \acronym{PCB} mount\\
		J1&1&&16-pin Euro-style terminal block\\
		J2&1&&Arduino stacking pin set\\
		J3&1&&2-pin Euro-style terminal block\\
		P0&1&&8-pin DIN plug\\
		Q0&1&&BC547 or 2N2222 NPN transistor\\
		R0--3&4&&10K resistor\\
		U0&1&\mouser{595-ULN2003AIDR}&\mc{ULN2003A} NPN Darlington array\\
		...
		for blue, green, and white LEDs. Check your LED datasheet to confirm. Examples: Mouser \mouser{652-4605X-1LF-100} (100$\Omega$)
		or \mouser{652-4605X-1LF-150} (150$\Omega$).
...
\section{Controller for Adafruit Matrices}
We obtained a number of pre-made \acronym{RGB} \led\ matrix displays from Adafruit which have a similar
(but not quite the same) control logic to our own design, which allows us to also control those boards using
only a slight variation in our hardware and firmware design.

The hardware to drive them is a small shield board that plugs onto an Arduino Due or Mega 2560. It incorporates the
same control circuitry as our original readerboard circuit described above, without the actual \led s themselves or
the discrete chips that drive the rows and columns of the display, since those are already on the pre-made displays.

This shield provides two ribbon cable connectors: J7, a 16-pin cable which sends the control signals to the matrix display,
and J8, a 10-pin cable that can drive an 8-\led\ discrete status display such as is on the busylight and our original readerboard.

The following describes the assembly of one of these shield boards.
\begin{table}
	\begin{tabular}{lrll}\toprule
		{\bfseries Part}&{\bfseries Qty}&{\bfseries Mouser}&{\bfseries Description}\\\midrule
		&1&&AdaShield Printed circuit board rev 0.0.0\\
		&1&&Arduino Due or Mega 2560\\
		C0&1&&\SI{100}{\micro\farad} electrolytic capacitor\\
		C1&1&\mouser{810-FA18X7R1H10400}&\SI{0.1}{\micro\farad} capacitor\\
		C2&1&\mouser{667-ECA-1HM100I}&\SI{10}{\micro\farad} electrolytic capacitor\\
		D0&1&&power diode, 1\,A\\
		J0&1&&Arduino stacking pin set\\
		J1a,b&2&&8-position terminal block\\
		J2,3&2&&2-position jumper header\\
		J4&1&&3-position jumper header\\
		J5,6&2&&2-position Euro-style terminal block\\
		J7&1&&16-position \acronym{IDC} header\\
		J8&1&\mouser{517-89110-0101}&10-position \acronym{IDC} header\\
		P0--2&3&&2-position jumper shunt\\
		Q0&1&&\mc{2N2222} or \mc{BC547} \mc{NPN} transistor\\
		R0--5&6&\mouser{603-MFR-25FRF5210K}&10K resistor\\
		R6&1&\mouser{604-MFR-24FRF521K}&1K resistor\\
		R$_{\text{TERM}}$&1&&120$\Omega$ resistor\\
		U0&1&&\mc{THVD1439} half-duplex RS-485 driver/receiver\\
		U1&1&&\mc{AT24C256} \acronym{EEPROM}\\
		U2&1&&\mc{ULN2803CDWR} octal NPN Darlington array\\
		&1&&9V 2A DC power supply\\
		\bottomrule
	\end{tabular}
	\caption{Bill of Materials for AdaSheild Board\label{fig:bom-ada}}
\end{table}

\subsubsection{AdaShield/QS Variation}
The author produced a variation on the AdaSheild boards for another project, but they may still be useful for
general use as an alternative to the above-described board in that it lacks a speaker output, but adds an I$^2$C alphanumeric
\led\ display output and expands the discrete \led\ output from 8 \led s to 14.

\begin{table}
	\begin{tabular}{lrll}\toprule
		{\bfseries Part}&{\bfseries Qty}&{\bfseries Mouser}&{\bfseries Description}\\\midrule
		&1&&AdaShield/QS Printed circuit board rev 0.0.0\\
		&1&&Arduino Due or Mega 2560\\
		C0&1&\mouser{667-ECA-1HM100I}&\SI{10}{\micro\farad} electrolytic capacitor\\
		C1&1&\mouser{810-FA18X7R1H10400}&\SI{0.1}{\micro\farad} capacitor\\
		D0&1&&power diode, 2\,A\\
		J0&1&&Arduino stacking pin set\\
		J1a,b&2&&8-position terminal block\\
		J2,3&2&&2-position jumper header\\
		J4&1&&3-position jumper header\\
		J5&1&&2-position Euro-style terminal block\\
		J6&1&&10-position \acronym{IDC} header\\
		J7,8&2&&16-position \acronym{IDC} header\\
		P0--2&3&&2-position jumper shunt\\
		R0--4&5&\mouser{603-MFR-25FRF5210K}&10K resistor\\
		R5&1&\mouser{604-MFR-24FRF521K}&1K resistor\\
		R6-8&3&&Resistor array, isolated, 4-resistors*\\
		R9,10&2&&Resistor*\\
		R$_{\text{TERM}}$&1&&120$\Omega$ resistor\\
		U0&1&&\mc{THVD1439} half-duplex RS-485 driver/receiver\\
		U1&1&&\mc{AT24C256} \acronym{EEPROM}\\
		U2,3&2&\mouser{595-ULN2003AIDR}&\mc{ULN2003A} NPN Darlington array\\
		&1&&9V 2A DC power supply\\
		\bottomrule
		\multicolumn{4}{p{\textwidth}}{\tiny*R6--10 each limit current to the discrete \led s. Choose their values appropriately
		for the supply voltage and current needs of the \led s to be driven.}
	\end{tabular}
	\caption{Bill of Materials for AdaSheild Board (QS Variant)\label{fig:bom-ada-qs}}
\end{table}
...
protocol used to control the hardware directly over a \acronym{USB} or \acronym{RS-485}
connection. However, you can set up a central server which is connected to all the
readerboard signs and busylight indicators in a small area (via \acronym{USB} and/or \acronym{RS-485}).
This allows clients to send commands to the server over the \acronym{LAN} for control of the readerboards.
...
it will transmit to the \acronym{RS-485} network a command starting with the hex byte sequence \z{BF}$_{16}$ \z{03}$_{16}$ \z{02}$_{16}$ \z{05}$_{16}$ \z{25}$_{16}$, or---if applicable---will send the message directly to any of devices 2, 5, and 37 which are directly connected via \acronym{USB}.

\section{Direct Host-to-Device Communication}
The host \acronym{PC} (which may or may not be running the server program) connects to one or more
readerboard and/or busylight devices by direct wire. The remainder of this chapter describes the
protocol used for this communication. If you consider the standard \acronym{ISO} \acronym{OSI} network
protocol stack, there are three layers of concern for us here, as illustrated in Figure~\ref{figure-layers}.

\begin{figure}
\begin{center}
\begin{bytefield}[endianness=little,bitwidth=0.05\textwidth]{9}
	\bitbox[]{1}{}&
	\bitbox[]{4}{\tiny ISO/OSI}&
	\bitbox[]{4}{\tiny Lumos}\\
	\begin{rightwordgroup}{Readerboard/Busylight}
	\bitbox[]{1}{7}&\bitbox{4}{Application}&\bitbox{4}{Application}
	\end{rightwordgroup}\\
	\bitbox[]{1}{6}&\bitbox{4}{\color{reservedslot}Presentation}\\
	\bitbox[]{1}{5}&\bitbox{4}{\color{reservedslot}Session}\\
	\begin{rightwordgroup}{USB or Lumos-compatible\\RS-485 network}
	\bitbox[]{1}{4}&\bitbox{4}{Transport}&\bitbox[lrt]{4}{Transport}\\
	\bitbox[]{1}{3}&\bitbox{4}{Network}&\bitbox[lrb]{4}{(LTP)}
	\end{rightwordgroup}\\
	\bitbox[]{1}{2}&\bitbox{4}{\color{reservedslot}Data Link}\\
	\bitbox[]{1}{1}&\bitbox{4}{Physical}&\bitbox{4}{Physical}
\end{bytefield}
\end{center}
	\caption{Protocol Stacks (ISO vs. Lumos Models)\label{figure-layers}}
\end{figure}

\section{Physical Layer}
At the lowest level, the serial bits are encoded and are carried over wires using either a direct \acronym{USB} connection
to the host or a wider-area \acronym{RS-485} serial network. We don't need to worry about that level of the communication
here.

It is worth noting, however, that the readerboard and busylight devices use half-duplex \acronym{RS-485} interfaces, so when
they are communicating over that network and are asked to send data back to the host, they will reverse the data direction
of these layers until that data are transmitted, then reverts back to receiving on the data channel.

\section{Transport Layer}
This layer is responsible for getting the signals to the correct devices. This layer adds some data packaging around the
message being sent to the device depending on the underlying physical connection type as described below.

\subsection{USB Transport Protocol}
Since each \acronym{USB} cable between the host and a device provides a single point-to-point connection, no addressing is
needed (since there can only be one device on the other end of the wire, the host already knows which device it's talking to
when sending messages on that \acronym{USB} port).

In this case, a single byte with value \z{04}$_{16}$ (character \z{\textasciicircum D}) is appended to the end of the
data packet.
...
If there is an error parsing or executing a command, the device will ignore
all subsequent input until a \z{\textasciicircum D} byte is received, whereupon it will
...
\subsection{RS-485 Transport Protocol}
The \acronym{RS-485} standard is a differential serial signal which can be run up to
1,000 meters over unshielded twisted-pair cables such as \acronym{CAT5} or \acronym{CAT6}.
Up to 32 devices (including the host itself) can ``tap into'' this serial bus at various
points (although physically this generally takes the form of ``daisy chaining'' connections
where a cable plugs each device into the next one in the chain, with terminators plugged into
the first and last devices).

Since multiple devices may exist on the same bus, they will all see the signals sent on that
bus and thus must at least understand those messages enough to differentiate their messages
from those destined for other devices even if they don't know how to fully interpret other
devices' command sets.

To accomplish this, the \acronym{RS-485} transport layer employed here uses a transport protocol
the author designed for his Lumos light control devices. By being compatible with this standard,
any set of readerboards, busylights, Lumos controllers, and any other devices following the same
standard may be mixed on the same \acronym{RS-485} network without conflict as long as they have
distinct addresses assigned to them.

All data sent over \acronym{RS-485} must conform to the following simple rules to be compatible
with the Lumos Transport Protocol (hereafter \ltp):
\begin{itemize}
	\item The first byte in a command packet has its \msb\ set (\z1).
		\begin{itemize}
			\item The lowest four bits of this byte specify the target device's address (0--15).
			\item The device may ignore this and all subsequent bytes if the address isn't the global address or its own.
			\item The remaining three bits specify the command to be executed on the target device.
			\item Devices MAY implement extended commands in subsequent bytes to allow more than 8 commands, and/or
				respond to global target addresses by decoding additional target IDs in subsequent bytes to allow
				for multiple targets per device and/or device IDs greater than 15, but this is optional.
		\end{itemize}
	\item All subsequent bytes in a command packet must have their \msb\ cleared (\z0).
	\item Devices MAY implement an escape code mechanism which does not otherwise violate the above rules to represent arbitrary
		8-bit values in the data packet, which will be translated as the packet passes through the transport layer.
	\item Following this header, the remaining bytes of the packet are the application-layer command as documented later
		in this document. No trailing bytes are added to the end of the packet (unlike \acronym{USB}).
\end{itemize}

The specifics of how these rules translate to various packet headers are described in detail below.

\subsubsection{Single Target or All Readerboards}
To send a command to a single sign, the \ltp\ packet header is a single byte encoded as:
\begin{center}
...
Alternatively, a command may be targeted to multiple signs; in this case the \ltp\ packet header
is a multi-byte code:
\begin{center}
	\begin{bytefield}[endianness=big]{8}
		\bitheader{0-7} \\
		\bitbox{1}{\z{1}}&
...
the initial \ltp\ header start byte. However, using the multiple target header, device addresses in the
...
For commands which send a response back to the computer, the RS-485 response data stream begins with the \ltp\ header
\begin{center}
	\begin{bytefield}[endianness=big]{8}
		\bitheader{0-7} \\
		\bitbox{1}{\z1}&
...
the sign(s). In other words, the \ltp\ packet header itself is all that is sent---the command payload is empty.
...
All subsequent \ltp\ bytes which follow the above binary headers \emph{must} have their
...
Once a command packet has been received and has been processed up through the transport layer, the remaining
contents of the packet are interpreted by the target device as the command to be carried out. In the descriptions
that follow, we will consider the matter of how the packet got to the device to have been fully resolved already
so no mention will be made of the transport protocol. Assume the \acronym{RS-485} header or \acronym{USB} trailer
...
\begin{table}
	\begin{center}
		\begin{tabular}{lcll}\toprule
			...
			\z{alert}&\z{!} & Briefly flash set of \led s & [3] \\
			\z{strobe}&\z{*} & Strobe \led s in Sequence & [1]\\
			\z{busy}&\z{?} & Query discrete \led\ status & [1] [2] [5]\\
			...
			\z{query}&\z{Q} & Query full device status & [2] [5]\\
			\z{text}&\z{T} & Display text on matrix & \\
			\z{alpha}&\z{V} & Manipulate segmented LED display & [6]\\
			\midrule
			...
			\noindent[6] On supported hardware only}
		\end{tabular}
		\caption{Summary of All Commands\label{tbl:commands}}
	\end{center}
\end{table}

\section{\z{\%T@}---Run Full Lamp Test Pattern}
\begin{center}
...
\section{\z{!}---Alert Signal on Discrete Lights}
\begin{center}
...
\end{bytefield}
\begin{Coding}
|/readerboard/v1/alert?a=|\Var*{ad}|&l=|\Var*{led$_0$}\Var*{led$_1$}\dots\Var*{led$_{n-1}$}
\end{Coding}
\end{center}
...
All of the \led s in the list are flashed briefly together to indicate an urgent status, after which
the previous state of the discrete \led\ display is restored.
...
\begin{Coding}
	...
	|&row=|\Var*{row}|&height=|\Var*{height}
\end{Coding}
\end{center}

In the second form, the \Var*{row} and \Var*{height} parameters define a region of the display that extends to the full width of the
display but includes \Var*{height} pixel rows starting at pixel \#\Var*{row} (with row \#0 being the topmost row of the device). In this
case, the command behaves as though this region is the only part of the sign that exists as far as where characters will be drawn and
scrolled through. {\color{red} Not yet implemented. May be limitations on how many simultaneous
transition effects may be playing at once, including scrolling text.}
...
\begin{table}
	\begin{center}
		\begin{tabular}{lll}\toprule
			...
			\z{\textasciicircum J}\Var*{pos} & \z{0A}\Var*{pos} & Move cursor down \Var*{pos} rows\\
			...
			\z{\textasciicircum U}\Var*{pos} & \z{15}\Var*{pos} & Move cursor up \Var*{pos} rows\\
...
this sign's address to \Var*{ad$'$}. (In the \acronym{HTTP} \acronym{API}, use ``\z{address=\_}'' to indicate this.)
...
This cannot be sent to the global device address, although you can request the server
to query the global address in which case it will individually poll each device.
...
The initial bytes ``\z{L0}'' identify the format of the data that follows. If the format changes
in a way that breaks the previous parsing rules, the ``\z0'' will be incremented according to the
conventions of 6-bit integer encoding as described in Table~\ref{tbl:int063}.
...
If the flasher has custom timings defined for it, a five-character block starting with ``\z{/}''
will appear at the start of the status and will contain the four timing values in the
same format as specified to the \z{F} command when they were set (q.v.).

In any case, \Var*{run} is the \ascii\ character ``\z{S}'' if the unit is
...
of the Unicode glyphs with codepoints less than 256.  See Tables~\ref{tbl:font0}--\ref{tbl:font3}
for a complete font glyph listing with codepoint assignments.

Font~3 is a double-height font which is really only useful on boards with 16 or more rows of \led s.
...
\begin{table}
	\begin{center}
		\begin{tabular}{r|c|c|c|c|c|c|c|c|l}
			...
			\emph{00x}&\tForbidden\tiny null&\tUnused&\tUnused&\tControl\tiny move&\tForbidden\tiny end&\tUnused&\tControl\tiny font&\tUnused&\multirow{2}{*}{\z{0}\emph{x}}\\\cline{1-9}
			\emph{01x}&\tControl\tiny back&\tUnused&\tControl\tiny down&\tControl\tiny color&\tControl\tiny forw&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{02x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tControl\tiny up&\tUnused&\tUnused&\multirow{2}{*}{\z{1}\emph{x}}\\\cline{1-9}
...
\begin{table}
	\begin{center}
		\begin{tabular}{r|c|c|c|c|c|c|c|c|l}
			...
			\emph{06x}&0&1&2&3&4&5&6&7&\multirow{2}{*}{\z{3}\emph{x}}\\\cline{1-9}
			\emph{07x}&8&9&:&;&\z<&=&\z>&?&\\\hline
			\emph{10x}&@&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{4}\emph{x}}\\\cline{1-9}
			\emph{11x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{12x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{5}\emph{x}}\\\cline{1-9}
			\emph{13x}&\tUnused&\tUnused&\tUnused&[&\tUnused&]&\textasciicircum&\_&\\\hline
			\emph{14x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{6}\emph{x}}\\\cline{1-9}
			\emph{15x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{16x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{7}\emph{x}}\\\cline{1-9}
			\emph{17x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{20x}&``&''&`&'&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{8}\emph{x}}\\\cline{1-9}
			...
			\emph{24x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{A}\emph{x}}\\\cline{1-9}
			\emph{25x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{26x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{B}\emph{x}}\\\cline{1-9}
			\emph{27x}&\tUnused&\tUnused&\tUnused&\tUnused&\sfrac14&\sfrac12&\sfrac34&\tUnused&\\\hline
%			\emph{30x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{C}\emph{x}}\\\cline{1-9}
%			\emph{31x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
%			\emph{32x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{D}\emph{x}}\\\cline{1-9}
%			\emph{33x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
...
%			\emph{37x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
		...
		\end{tabular}
	\end{center}
	\caption{Font Table for Font \#3\label{tbl:font3}}
\end{table}
\begin{table}
	\begin{center}
		\begin{tabular}{r|c|c|c|c|c|c|c|c|l}
			...
			\emph{02x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tControl\tiny up&\tUnused&\tUnused&\multirow{2}{*}{\z{1}\emph{x}}\\\cline{1-9}
			\emph{03x}&\tControl\tiny hex&\tUnused&\tUnused&\tForbidden\tiny esc&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{04x}&&\tUnused&$\tUnused$&$\tUnused$&\tUnused&\tUnused&\tUnused&\tUnused&\multirow{2}{*}{\z{2}\emph{x}}\\\cline{1-9}
			\emph{05x}&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\\\hline
			\emph{06x}&0&1&2&3&4&5&6&7&\multirow{2}{*}{\z{3}\emph{x}}\\\cline{1-9}
			\emph{07x}&8&9&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused&\tUnused\\\hline
			&\z{8} &\z{9} &\z{A} &\z{B}
			&\z{C} &\z{D} &\z{E} &\z{F}
		\end{tabular}
	\end{center}
	\caption{Font Table for Fonts \#4--\#5\label{tbl:font4}\label{tbl:font5}}
\end{table}
...
\z{\#} or \z{b} to indicate a sharp ($\sharp$) or flat ($\flat$) note, respectively, followed by a digit \z0--\z8 to indicate the octave. The notes range from \z{B0} (B$_0$ at 31\,Hz)
to \z{D\#8} (D$_8^{\sharp}$ at 4.978\,kHz). A rest is indicated by ``\z{R}'' as the note letter.
...
is ``\z{G432E532C564}'' (note G$_4$, \z{32}$_{16}$\,cs=0.50\,s, note E$_5$, \z{32}$_{16}$\,cs=0.50\,s, note C$_5$, \z{64}$_{16}$\,cs=1.00\,s).
Duration codes range from \z{00} (\z0$_{16}$=0$_{10}$\,cs=0.00\,s) to \z{FF} (\z{FF}$_{16}$=255$_{10}$\,cs=2.55\,s),
in increments of 0.01\,s.

The letters may be upper- or lower-case (the letter \z{b} is distinguished as a note name vs.\ a ``$\flat$'' half-step shift indicator from context).

Some devices are not able to play notes but can sound a beep at a fixed frequency (typically 490\,Hz or just ever so slightly flat of B$_4$
for SparkFun Pro Micro and Adruino Mega 2560 microcontrollers and 1\,kHz or between B$_5$ and C$_6$ for Arduino Due). These will play that single
...
value from \z{00}$_{16}$ (fully off) to \z{FF}$_{16}$ (fully on). (In the \acronym{HTTP} \acronym{API}, as integers 0--255.)
...
Not all \led s are dimmable, depending on the specific hardware capabilities. All \led s on readerboard models from revision 3.\Var{x} and after. On the busylight
hardware, only the third, fifth, and seventh, \led\ tiers (usually assigned amber, red, and white colors) are dimmable.

Note that the readerboards were designed to run best at full brightness. Dimming the matrix display too much may result in the display
appearing to flicker too much. This effect is most pronounced with white content, a little less so with yellow, cyan, and magenta.
...
\begin{Coding}
	...
	|&up=|\Var*{sec}|&on=|\Var*{sec}|&down=|\Var*{sec}|&off=|\Var*{sec}
\end{Coding}
\end{center}
...
Optionally, a block of timing values may be given before the block of \Var*{led} values.
This is introduced by a ``\z{/}'' character and consists of four 6-bit encoded integer values
giving the time interval over which the light should fade up to full brightness, the time to
remain on, the interval over which to dim down, and the time to remain off. These values
are in units of \sfrac{1}{10}th of a second (thus ranging from 0--6.4 seconds). For \led s
which do not support dimming, the values of \Var*{up} and \Var*{down} are treated as if they
were zero, instantly turning on or off the \led. (In the \acronym{HTTP} \acronym{API},
the \Var*{sec} values are floating-point numbers of seconds; e.g., a value of 1.2 seconds would be
expressed in the \acronym{API} as ``\z{up=1.2}'' and in the hardware protocol as ``\z{<}'').

If you have set a dimmer level for any of the \led s, then the maximum brightness that they will
ramp up to and down from will be that dimmer level. If the dimmer is set very low, then the
effect of \Var*{up} and \Var*{down} won't be too noticeable since you didn't leave it much
room to dim up and down within that range.
...
\begin{Coding}
	...
	\Var*{rgb$_4$}\Var*{rgb$_5$}\Var*{rgb$_6$}\Var*{rgb$_7$}\dots
\end{Coding}
\end{center}
...
The value \Var*{n} is an integer encoded as per Table~\ref{tbl:int063}, and
is drawn in the far-right column of the matrix display, as a column of \Var*{n} lights
stacked up from the bottom row (a value of 0 results in no lights).
All existing matrix data are scrolled left one column.

In earlier versions of the readerboard hardware, devices with 27 or more rows weren't supported. Now that they are,
the value ``\z{K}'' makes this command ambiguous since \z{HK} could be the start of a bar graph command with explicit color
codes, or a single point with value 27. To resolve the ambiguity, an optional ``\z{\textasciitilde}'' character may be
sent after the initial \z{H} to explicitly indicate that the following byte is a data value. Thus, \z{HK} is always
the start of an explicitly-colored bar, and \z{H\textasciitilde K} is how you would send a data bar of 27 pixels
in height. You {\bfseries may} start all data points with \z{H\textasciitilde} but you only {\bfseries must} do so
if the value encodes to \z{K}.

In the second form, it takes a number of \Var*{rgb} values (see Table~\ref{tbl:rgbcolors}) which give the color to light up
each row in the bar-graph column. Unlike other drawing commands, but to make the most logical sense for histograms, \Var*{rgb$_0$} refers to the bottom-most
\led\ in the column, so the number of color values in the list is the number of data points being graphed.
...
\begin{Coding}
	...
	|&trans=|\Var*{trans}|&image=|\Var*{R coldata}\dots\z{\$}\Var*{G coldata}\dots\\
	\z{\$}\Var*{B coldata}\dots\z{\$}\Var*{flashdata}|&height=|\Var*{height}
\end{Coding}
\end{center}
...
each of which is a two-digit \ascii\ hexadecimal value for each byte of height
(the number of bytes of height are given by the \Var*{height} value), with the 
least-significant bit of the first such digit pair representing the top row of the matrix.
...
%	\bitheader{0-8} \\
	...
	\bitbox{2}{\Var*{width}} &
	\bitbox{2}{\Var*{height}} &
	\bitbox[tbl]{2}{\Var*{R coldata}} &
	\bitbox[tbr]{2}{$\cdots$} &
	\bitbox{1}{\z{\$}}\\
	\bitbox[tbl]{2}{\Var*{G coldata}} &
	\bitbox[tbr]{1}{$\cdots$} &
	\bitbox{1}{\z{\$}}&
	\bitbox[tbl]{2}{\Var*{B coldata}} &
	\bitbox[tbr]{2}{$\cdots$} &
	\bitbox{1}{\z{\$}} \\
	\bitbox[tbl]{2}{\Var*{flashdata}} &
	\bitbox[tbr]{1}{$\cdots$} &
	\bitbox{1}{\z{\$}} &
	\bitbox{1}{\Var*{alen}}&
	\bitbox{2}{\Var*{adata}$\cdots$}&
	\bitbox{1}{\z{\$}}&
	\bitbox{1}{\z{\textbackslash n}}
\end{bytefield}
\end{center}

The leading ``\z{Q0}'' indicates the format of the data that follows. If a change is introduced
that breaks the parsing rules needed to understand the response, the ``\z0'' will be incremented
as documented in Table~\ref{tbl:int063} for 6-bit integer encoding.
...
``\z{M}'' for \Var*{width}$\times$\Var*{height} matrix display hardware with monochrome \led s, 
``\z{C}'' for \Var*{width}$\times$\Var*{height} \acronym{RGB} color display boards, or
``\z{Q}'' for the ``QS'' variant of color display boards with expanded discrete \led\ support and
alphanumeric segmented displays.
Note that \Var*{width} and \Var*{height} are encoded 12-bit integers, where the most significant 6 bits
are sent first in each pair and the full encoded value is one less than the actual width or height.
Thus, dimensions of 1--4,096 pixels in each direction may be reported.
...
For devices which have 14-segment alphanumeric \led\ displays, the number of supported digits is sent as
a 6-bit integer value \Var*{alen} (or 0 if there are no such displays on this device). This is followed by
a number of four-digit big-endian hex words, one per \led\ digit, which indicate the currently-lit segments on the display.
In each word, the bits correspond to segments a, b, c, \dots, l, m, n, and the decimal point, from bit 0 to bit 14.
Bit 15, the \msb, is always clear.
...
In the second form, the \Var*{row} and \Var*{height} parameters define a region of the display that extends to the full width of the
display but includes \Var*{height} pixel rows starting at pixel \#\Var*{row} (with row \#0 being the topmost row of the device). In this
case, the command behaves as though this region is the only part of the sign that exists as far as where characters will be drawn,
transition effects performed, previous data cleared, etc. {\color{red} Not yet implemented. May be limitations on how many simultaneous
transition effects may be playing at once, including scrolling text.}

The \Var*{align} code indicates how the text is aligned on the display using one of the codes
shown in Table~\ref{tbl:alignments}. 
\begin{table}
	\begin{center}
		\begin{tabular}{cl}\toprule
...
If \Var*{merge} is ``\z{M}'' then the text is drawn without disturbing any existing pixels around it, adding
to what is already there. If it is ``\z{.}'' then everything to the right of the cursor is cleared before
drawing the text. (Alternatively, this may clear everything to the left of the cursor or the entire screen, depending
on the alignment mode in use.)
...
\section{\z{V}---Manipulate Segmented LED Display}
For units which include 14-segment alphanumeric character displays, this set of commands manipulates
what is displayed there. There are three distinct subcommands which are grouped together here:
...
\begin{Coding}
	|/readerboard/v1/alpha?a=|\Var*{ad}|&pos=|\Var*{pos}|&img=|\Var*{bool}|&t=|\Var*{text}
\end{Coding}
\end{center}

Starting with character position \Var*{pos} (with 0 being the leftmost character), write the
characters of the \Var*{text} string into successive positions. Any character positions to the
left of \Var*{pos} or to the right of the \Var*{text} are left unchanged. 

If \Var*{pos} is the special code ``\z{\#}'', the entire display is replaced with \Var*{text}, which is scrolled
across the display once until it is fully visible, then the scrolling stops. If \Var*{pos} is ``\z{\%}'', the behavior
is the same but the scrolling loops forever instead of stopping.

If \Var*{img} is ``\z{.}'', then \Var*{text} is an \acronym{ASCII} string of characters which will be rendered
using the device's built-in character set.  A period (``\z.'') character
in \Var*{text} does not consume an additional character if it follows another character in the string;
in this case the decimal point on the previous character's segmented display will be lit.

However, if \Var*{img} is ``\z{I}'', then \Var*{text} specifies the individual set of \led\ segments to light up
for each affected character position. Each character to display is sent as a 
two-byte big-endian value expressed as a four-digit hex value that provides
a bitmap of all fifteen \acronym{LED} segments that should be illuminated for that character.

\begin{figure}
	\begin{center}
		\includegraphics[width=1in]{images/14-segment} \\
		{\footnotesize Image: Wikimedia Commons (public domain)}
		\caption{14-Segment LED Alphanumeric Display\label{fig:14seg}}
	\end{center}
\end{figure}

Referring to the diagram in Figure~\ref{fig:14seg}, the \acronym{LSB} (bit 0) of each word is set (\z1) if segment ``a'' is lit,
bit 1 for segment ``b'', and so on, up to bit 14 for the decimal point. The \msb, bit 15, is always clear (\z0).
...
\begin{Coding}
	|/readerboard/v1/alpha-clear?a=|\Var*{ad}|&pos=|\Var*{pos}|&clearto=|\Var*{end}
\end{Coding}
\end{center}

Turns off character positions starting with \Var*{pos}, through \Var*{end}, inclusive.
...
\begin{Coding}
	|/readerboard/v1/alpha-clear?a=|\Var*{ad}
\end{Coding}
\end{center}

Turns off all character positions.
...
\begin{table}
	\begin{center}
		\begin{tabular}{ll}\toprule
			...
			\z{\{\textasciicircum}[$\pm$]\Var*{n}\z{\}}&Reposition cursor up [down] by \Var*{n} rows\\
...
\section{Adafruit Matrix Versions}
There are two experimental versions of readerboard sheilds which drive the various maxrix boards available from Adafruit.\footnote{And
probably others, but that's just where the author got his from.} These use a 16-pin ribbon cable which employs signalling that is similar
in principle (although not identical) to our original design. Our compatible shield boards provide these signals on their J7 connector:

\begin{center}
	\begin{tabular}{rl|rl}\toprule
		\bfseries Pin&\bfseries Signal &
		\bfseries Pin&\bfseries Signal \\\midrule
		 1&R$_1$ red data      & 2&G$_1$ green data\\
		 3&B$_1$ blue data     & 4&Ground\\
		 5&R$_2$ red data      & 6&G$_2$ green data\\
		 7&B$_2$ blue data     & 8&E (A$_4$) row select or ground\\
		 9&A (A$_0$) row select&10&B (A$_1$) row select\\
		11&C (A$_2$) row select&12&D (A$_3$) row select or ground\\
		13&Clock               &14&Latch \\
		15&Output Enable       &16&Ground \\\bottomrule
	\end{tabular}
\end{center}

The standard adafruit-compatible shield supports eight busylight-like discrete \led\ outputs as well, but instead of them being soldered
to the board as they would be on the original readerboard, ribbon cable connector J8 connects to external \led s, which are assumed to
have their own current-limiting resistors:
\begin{center}
	\begin{tabular}{rl|rl}\toprule
		\bfseries Pin&\bfseries Signal &
		\bfseries Pin&\bfseries Signal \\\midrule
		1&+9V common anode power &2&Ground\\
		3&L7 cathode & 4&L6 cathode\\
		5&L5 cathode & 6&L4 cathode\\
		7&L3 cathode & 8&L2 cathode\\
		9&L1 cathode &10&L0 cathode\\\bottomrule
	\end{tabular}
\end{center}

The remaining external connections, including 
J1 to RS-485 network,
J5 to +9V power supply,
and
J6 to external speaker,
are as for the original readerboard unit.

\subsection{QSCC Variant}
A slight variation of the Adafruit-compatible shields, the so-called ``QSCC'' model, is used for game applications in another project by the author,
but may be useful in a variety of other situations as well. This is the same as what is listed above, except it does not offer a speaker output,
and adds additional discrete \led\ outputs on J8 with the following ribbon cable pinout:

\begin{center}
	\begin{tabular}{rl|rl}\toprule
		\bfseries Pin&\bfseries Signal &
		\bfseries Pin&\bfseries Signal \\\midrule
		1&L0 cathode & 2&L1 cathode\\
		3&L2 cathode & 4&L3 cathode\\
		5&L4 cathode & 6&L5 cathode\\
		7&L6 cathode & 8&L7 cathode\\
		9&L8 cathode &10&L9 cathode\\
		11&L10 cathode &12&L11 cathode\\
		13&L12 cathode &14&L13 cathode\\
		15&+9V common anode power&16&+9V common anode power\\\bottomrule
	\end{tabular}
\end{center}
The shield includes on-board current-limiting resistors for these \led s so they don't have to include their own.

It also adds J6 which exposes two identical I$^2$C connections indended to drive alphanumeric displays such as the
four-character Adafruit ``backpack'' units.  These have a ribbon cable pinout as follows:
\begin{center}
	\begin{tabular}{rl|rl}\toprule
		\bfseries Pin&\bfseries Signal &
		\bfseries Pin&\bfseries Signal \\\midrule
		1&V$_{\text{HI}}$ (typ. +5V) & 2&V$_{\text{IO}}$ (logic voltage)\\
		3&Ground                     & 4&SDA\\
		5&SCL                        & 6&SCL\\
		7&SDA                        & 8&Ground\\
		9&V$_{\text{IO}}$            &10&V$_{\text{HI}}$\\\bottomrule
	\end{tabular}
\end{center}

This pin arrangement allows you to wire it to an \acronym{IDC} ribbon cable connector, split the ribbon cable in two, sending each half to a different
I$^2$C display. Alternatively, you could wire this to a wire-to-board socket with discrete wires which can be bundled by column or sequentially.
...
\section{Power}
The readerboard units require an external 9\,V~DC power supply in order to sustaiin the power requirements of all of the
\led s it uses. Always be sure that this power supply is connected and operating before plugging in the readerboard to \acronym{USB}.
Otherwise, you may overload the \acronym{USB} port on the host computer and/or the Arduino microcontroller as it tries to power the readerboard by itself.

Similarly, busylight units are designed to be powered from the \acronym{USB} connection \emph{or} from an external power source (such as a
pair of power wires on the \acronym{CAT6} cable used for \acronym{RS-485} signals) but not both at the same time.

Readerboard \acronym{PCB} revisions starting with 3.4.1 introduced diode D520 to mitigate this issue, so the \led s don't end up being powered by the
Arduino. Busylight \acronym{PCB} revisions starting with 2.2.0 did the same with the introduction of D0.
...
#define SERIAL_VERSION_STAMP "V3.4.1$R2.3.8$"
//                             \___/  \___/
//                               |      |
//                  Hardware version    |
//                         Firmware version
...
\noindent\includegraphics[width=\textheight,angle=90]{images/adashield_schematic.png}
\noindent\includegraphics[width=\textheight,angle=90]{images/adashield_qs_schematic.png}
...
\noindent\includegraphics[width=.48\textwidth]{images/adashield-assembly-0.png}
\noindent\includegraphics[width=.48\textwidth]{images/adashield-assembly-1.png}

\noindent\includegraphics[width=.48\textwidth]{images/adashield_qs-assembly-0.png}
\noindent\includegraphics[width=.48\textwidth]{images/adashield_qs-assembly-1.png}
