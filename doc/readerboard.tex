...
%	\bitheader{0-3} \\
	...
	\bitbox{1}{\z{*}} &
	\bitbox[tlb]{1}{\Var*{light$_0$}} &
	\bitbox[tb]{1}{\Var*{light$_1$}} &
	\bitbox[tb]{1}{\Var*{light$_2$}} &
	\bitbox[tb]{1}{$\cdots$} &
	\bitbox[tbr]{1}{\Var*{light$_n$}} &
	\bitbox{1}{\z\$}&
	\bitbox{1}{\z{\textasciicircum D}}
\end{bytefield}

Each \Var*{light} value is an \ascii\ digit character corresponding to a discrete
...
\begin{table}
	\begin{center}
		\begin{tabular}{lll}\toprule
			...
			\bottomrule
		\end{tabular}
		\caption{Control Codes Allowed in String Values\label{tbl:controlcodes}}
	\end{center}
\end{table}
...
\noindent\begin{bytefield}[endianness=little,bitwidth=0.11111\textwidth]{9}
	\bitheader{0-8} \\
	\bitbox{1}{\z{L}} &
	\bitbox[tbl]{1}{\Var*{light$_0$}} &
	\bitbox[tb]{1}{\Var*{light$_1$}} &
	\bitbox[tb]{1}{\Var*{light$_2$}} &
	\bitbox[tb]{1}{\Var*{light$_3$}} &
	\bitbox[tb]{1}{\Var*{light$_4$}} &
	\bitbox[tb]{1}{\Var*{light$_5$}} &
	\bitbox[tb]{1}{\Var*{light$_6$}} &
	\bitbox[tbr]{1}{\Var*{light$_7$}} \\
	\bitbox{1}{\z{F}} &
...
The \Var*{light} values are as given to the \z{F} or \z{*}
...
Up to 64 \Var*{light} codes may be listed. The sign will cycle through the sequence, lighting each
...
If only one \Var*{light} is specified, that light will be flashed on and off.
Setting an empty
...
\href{https://semver.org}{semver.org}.}

The \Var*{serial} field is a variable-width string which was set when the firm\-ware was
...
each time). Serial numbers 0--299 are reserved for the original author's use.
...
\led s. The single \led\ indicated by \Var*{light} is turned on. See Table~\ref{tbl:lightcodes}. Note that if a strobe sequence is running (via a previous \z{*} command),
it remains running.
