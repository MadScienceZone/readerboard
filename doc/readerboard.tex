...
\begin{center}\bfseries
	For readerboard hardware (\acronym{PCB}) version 3.2.3, schematic version 3.2.3, and firmware version 0.0.0.
\end{center}
...
\section{Version 2.0}
This version is based on the 1.0 design, but shrunk to a board size of about 18\sfrac{3}{16}$\times$5\sfrac{3}{16}$''$
...
\section{Version 3.2}
The current version of the hardware expands on the version 2.1 design by replacing the single-color \acronym{LED}s with
...
As of this writing, the current revision of this board is schematic version 3.2.3 and \acronym{PCB} version 3.2.3.
The improvement over 3.2.2 is a cut-out to accommodate the connectors on the Arduino board and the addition of R51.
Otherwise, 3.2.2 boards fully function the same as the 3.2.3 revision.
...
The boards before \acronym{PCB} revision 3.2.2 are not supported by the firmware. They were prototype ideas
...
\begin{enumerate}
	...
	\item Solder the 2-pin headers J2 and J3.
...
would transmit to the \acronym{RS-485} network the command starting with the hex byte \z{92}.

If multiple devices are targeted, say with a message containing the field \z{a=2,5,37} with address 15 (e.g., \z{F}$_{16}$) configured as the global address \Var{ad$_G$},
it will transmit to the \acronym{RS-485} network a command starting with the hex byte sequence \z{BF 03 02 05 25}.
...
\section{Connectors for Version 3.2 Boards}
...
The single external connector on version 3.2.3 boards is an 8-pin screw terminal block. This accepts a +9\,V DC power input
...
The following page includes the schematics for the current 3.2.1 version of the readerboard \acronym{PCB}
and the board assembly diagram, showing where the components are placed.
\newpage
\noindent\includegraphics[width=\textheight,angle=90]{../hardware/matrix64x8_rgb_300_schematic.png}
\noindent\includegraphics[height=\textheight]{../hardware/matrix64x8_rgb_300_assembly-0.png}
\noindent\includegraphics[height=\textheight]{../hardware/matrix64x8_rgb_300_assembly-1.png}
\indexintoc
